...
Options:
  ...
  --time <ms>        AI time per move (default: 1000)
...
function buildDefaultReportFilename(args: Args): string {
  ...
  return sanitizeFilename(
    `grr_${args.opening}_${args.matches}m_${args.sessions}s_${args.cols}x${args.rows}_score${args.score}_t${args.timeMs}_${stamp}.json`
  );
}

function buildRunKey(args: Args): string {
  const base = sanitizeFilename(
    `grr_${args.opening}_${args.matches}m_${args.sessions}s_${args.cols}x${args.rows}_score${args.score}_t${args.timeMs}`
  );
...
async function main(): Promise<void> {
  ...
  const browser = await chromium.launch({
    headless: false,
    args: ['--disable-popup-blocking', '--disable-infobars', '--disable-extensions', '--disable-sync', '--no-first-run'],
  });
  ...
  try {
    ...
    // Open the arena in a popup-style window (minimal chrome).
    const openScript = `(() => {
    ...
    const fallbackSavePromise = (async () => {
      try {
        await arenaPage.waitForFunction('window.__grrArenaDone === true', { timeout: 0 });
        if (saved) return;
    ...
    const checkpointWriteOnce = async (): Promise<void> => {
      try {
        const payload = (await arenaPage.evaluate('window.__grrArenaGetCheckpoint ? window.__grrArenaGetCheckpoint() : null')) as unknown;
        if (!payload) return;
        const json = JSON.stringify(payload, null, 2) + '\n';
